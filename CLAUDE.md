...
- **Porțile se omit silențios fără `runs/`**: testul-ancoră
  ...
  — apoi `eval --min-anchors 35` e exact poarta din CI. Fixture-urile
...
- **`data/<an>/manifest.json` e registrul de progres**, scris de comanda
  `batch` în timpul conversiilor — nu-l edita concurent cu un batch în mers.
  `triage` și `convert` (unitar) nu scriu în el.
- **LLM = bani reali**: `--llm repair/full` apelează API-uri plătite; dă
  întotdeauna `--max-llm-cost` și preferă preseturile din
